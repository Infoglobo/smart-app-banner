...
var SmartBanner = function (options) {
  ...
  var useDefaultBanner = isMobileSafari && !this.options.ios_app_id
  ...
  if (useDefaultBanner || runningStandAlone || userDismissed || userInstalled) {
    return
  }
...
SmartBanner.prototype = {
  ...
  create: function () {
    ...
    sb.innerHTML =
      ...
      '<span class="smartbanner-icon" style="background-image: url(' +
      icon +
      ')"></span>' +
    ...
    q('.smartbanner-button', sb).addEventListener(
      'click',
      this.install.bind(this),
      false
    )
    q('.smartbanner-close', sb).addEventListener(
      'click',
  ...
  close: function () {
    ...
    cookie.set(this.appId + '-smartbanner-closed', 'true', {
      path: '/',
      expires: new Date(
        Number(new Date()) + this.options.daysHidden * 1000 * 60 * 60 * 24
      )
    })
    if (typeof this.options.close === 'function') {
      return this.options.close()
  ...
  install: function () {
    ...
    cookie.set(this.appId + '-smartbanner-installed', 'true', {
      path: '/',
      expires: new Date(
        Number(new Date()) + this.options.daysReminder * 1000 * 60 * 60 * 24
      )
    })
    if (typeof this.options.close === 'function') {
      return this.options.close()
  ...
  parseAppId: function () {
    var meta = q('meta[name="' + this.appMeta + '"]') || this.options.ios_app_id
    if (!meta) {
      return
    }
